    ...
    if (isFirstMessage) {
      if (initialUserPromptChoices) {
        const userPromptChoicesMessage: ORMessage = {
          role: "assistant",
          content: initialUserPromptChoices.map(
            (choice) => `[${choice}](?userPrompt=${encodeURIComponent(choice)})`
          ).join(" | "),
        };
      ...
      if (lastMessage.role === "assistant" && typeof lastMessage.content === "string") {
        const { suggestedPrompts, newContent } = parseSuggestedPrompts(lastMessage.content);
        if (suggestedPrompts && suggestedPrompts.length > 0) {
          const userPromptChoicesMessage: ORMessage = {
            role: "assistant",
            content: suggestedPrompts.map(
              (choice) => `[${choice}](?userPrompt=${encodeURIComponent(choice)})`
            ).join(" | "),
          };
          ret.push(userPromptChoicesMessage);
...
function parseSuggestedPrompts(content: string): {
  ...
  if (startIndex !== -1 && endIndex !== -1) {
      ...
      if (prompts.length > 0) {
          ...
          newContent = content.slice(0, startIndex) +
              content.slice(endIndex + endTag.length);
      }
  }
